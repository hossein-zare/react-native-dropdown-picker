...
class DropDownPicker extends React.Component {
  constructor(props) {
    ...
    if (!props.multiple) {
      if (props.defaultValue || props.defaultValue === 0) {
        ...
        props.items.filter((item) => item.hasOwnProperty('selected') && item.selected === true)
          .length > 0
      ) {
      ...
      if (
        ...
        items = props.items.filter(
          (item, index) => item.hasOwnProperty('selected') && item.selected === true,
        );
      }
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    if (
      ...
      JSON.stringify(props.defaultValue) !== JSON.stringify(state.props.defaultValue)
    ) {
      let items = [];
      if (
        props.defaultValue &&
            ...
            (async () => {
              for (const value of defaultValue) {
                await new Promise((resolve, reject) => {
                  resolve(this.select(items.find((item) => item.value === value)));
                });
              }
            })();
          } else {
      ...
      <View
        ...
        <TouchableOpacity
          ...
          <View
            ...
            <View style={{justifyContent: 'center'}}>{item.icon && item.icon()}</View>
            <Text
              style={[
          ...
          {this.state.props.multiple &&
            this.state.choice.findIndex((i) => i.label === item.label && i.value === item.value) >
              -1 &&
        ...
        {children.length > 0 && (
          <View
            ...
            {children.map((child, childIndex) =>
              this.renderItem(child, this.concatNums(index, childIndex), children.length),
            )}
          </View>
        )}
      ...
      <View
        ...
        <View
          ...
          <ScrollView
            ...
            {...scrollViewProps}>
            {items.length > 0 ? (
              items.map((item, index) => this.renderItem(item, index, items.length))
            ) : (
